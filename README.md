...
- **六种镜像方向**：左对称、右对称、上对称、下对称、左上对称（取源图左上角，沿主对角线斜切）、右下对称（取源图右下角，沿副对角线斜切）
...
| `tests/mirror-harness.html` | 静态图镜像算法断言：六方向尺寸 / 水平垂直对称性 / 对角线斜切对称（tl 主对角线、br 副对角线）/ 斜切无透明 / 保留角内容 / keepOriginalSize 等比缩放 / maxEdge 导出上限 / 背景色填充 / 方向区分等 52 项 |
...
- **对角线方向修正**：左上对称与右下对称按"镜像轴端点"语义一一对应——左上对称取源图左上角 c×c、沿主对角线（\）斜切（右下三角保留原内容），右下对称取源图右下角 c×c、沿副对角线（/）斜切（右上三角保留原内容），镜像轴叠加层同步区分（tl 画主对角线、br 画副对角线）；首次修正误将两者统一为主对角线，已再次修正为上述语义
- **预览图完整显示**：原图与镜像结果图高度改为自适应容器（`max-height: 100%`），不再受固定 320px 上限约束，窄窗口下边缘不再被裁切
- **间距优化**：方向按钮图标与文字间距 3px → 6px、按钮内边距同步加大，图标文字不再拥挤
- **测试更新**：`tests/mirror-harness.html` 对角用例重写（tl 断言主对角线对称 + 右下三角保留红块，br 断言副对角线对称 + 右上三角保留黄块），51 → 52 项
