...
- **六种镜像方向**：左对称、右对称、上对称、下对称、左上对称（副对角线）、右下对称（主对角线）
...
| `tests/mirror-harness.html` | 静态图镜像算法断言：六方向尺寸 / 水平垂直对称性 / 对角线转置与空白象限 / 镜像块位置 / keepOriginalSize 等比缩放 / maxEdge 导出上限 / 背景色填充 / 方向区分等 44 项 |
...
- **对角线镜像**：新增「左上对称（↖）/ 右下对称（↘）」两个方向，镜像方向从 4 → 6 个；取方形裁剪区（边长 = min(宽,高) × 比例），右下对称输出「原块 + 转置块」沿主对角线展开，左上对称输出「转置(180°)块 + 原块」沿副对角线展开，右上/左下象限透明；GIF 与静态图通用
...
- **测试扩充**：`tests/mirror-harness.html` 新增对角线尺寸 / 转置对称 / 空白象限 / 内容位置 / maxEdge 上限断言，25 → 39 项
